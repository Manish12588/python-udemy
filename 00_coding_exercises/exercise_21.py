...
You are building an Order Invoice Generator system for a restaurant.
The function should be versatile enough to accept a customer's name, a flexible number of ordered items,
and a set of dynamic extra charges (like tax, service charge, etc.).
...
# str ="Guest" is the default value
def generate_invoice(
    ...
    if charges:
        ...
        for label, value in charges.items():
            invoice_line.append(f"{label.capitalize()}: {value}")
            total += value
    invoice_line.append(f"Total Amount Due: {total}")
    return "\n".join(invoice_line)


amit = generate_invoice("Amit", "Burger", "Fries", tax=50.0, service=20.0)
riya = generate_invoice("Riya", tax=30.0)
defualtperson = generate_invoice()
john = generate_invoice("John", "Pizza", "Coke")
