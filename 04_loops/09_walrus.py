# Simple program to calculate remainder
value = 13
remainder = value % 5

if remainder:
    print(f"Not Divisiable, remainder is {remainder}")


# Same remainder program to calculate via walrus operator

if remainder := value % 5:
    print(f"Not Divisiable, remainder is {remainder}")


# Another way to use walrus operator

available_sizes = ["small", "medium", "large"]

if (requested_size := input("Enter your chai cup size: ")) in available_sizes:
    print(f"Serving {requested_size} chai.")
else:
    print(f"Size {requested_size} is unavailable.")

# Another way to use walrus operator
flavors = ["masala", "ginger", "lemon", "mint"]
print(f"Available flavors: {flavors}")

while (flavor := input("Choose your flavor: ")) not in flavors:
    print(f"Sorry, {flavor} is not available")

print(f"You choose {flavor} chai")
